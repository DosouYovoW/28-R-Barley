...
## Lodging index data exploration
...
#### Performance of feed barley varieties at different locations
...
```{r}
p4 <- data_graph %>%
  filter(trt %in% c("lodging_index_1", "lodging_index_2", "lodging_index_3", "lodging_avg")) %>%
  mutate(trt = recode(trt,
                      "lodging_index_1" = "Soft Dough",
                      "lodging_index_2" = "Hard Dough",
                      "lodging_index_3" = "Physiological Maturity",
                      "lodging_avg" = "Average"
))  %>% 
  mutate(trt = factor(trt, levels = c("Soft Dough", "Hard Dough", "Physiological Maturity", "Average")),
         variety = fct_relevel(variety, "CDC_Austenson", "AB_Hague", "Esma")) %>% 
  ggplot(aes(variety, mean_r)) +
  geom_bar(aes(fill = variety), stat = "identity", show.legend = TRUE, width = 0.5) +
  facet_grid(location ~ trt, scales = "free_y") + #, scales = "free_y"
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2)+
  geom_text(aes(label = gr, y = mean_r + SE_r + 0.1), vjust = -0.3, size =5, show.legend = FALSE)+
  scale_fill_manual(values = c("#336600", "#999999", "#996600"),
                  labels = c("CDC Austenson", "AB Hague",  "Esma"))+
  ...
  labs(x = "Variety", y = "Lodging Index (%)", fill = ""#, title = "d) Lodging at different growth stages (Average of three years)"
       )
p4
ggsave("Final_Report/figure/Variety_impact_lodging.png", width = 8, height = 4, dpi = 200)
```

#### Performance of feed barley varieties at different locations and year
...
cleaned_data_imputed_LOCATION_2022 <- cleaned_data_imputed %>% filter(year == 2022) 

cleaned_data_imputed_LOCATION_2023 <- cleaned_data_imputed %>% filter(year == 2023) 

cleaned_data_imputed_LOCATION_2024 <- cleaned_data_imputed %>% filter(year == 2024) 


locations <- c("Vermilion", "Falher", "Lethbridge")  
locations_2 <- c("Vermilion", "Lethbridge") 
...
# Loop over all combinations of locations and treatments
all_results_LOCATION_2022 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_LOCATION_2022, ..1, ..2)) %>% 
  mutate(year = 2022)

all_results_LOCATION_2023 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_LOCATION_2023, ..1, ..2))%>% 
  mutate(year = 2023)

all_results_LOCATION_2024 <- crossing(loc = locations_2, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_LOCATION_2024, ..1, ..2))%>% 
  mutate(year = 2024)

all_results_LOCATION_year <- all_results_LOCATION_2022 %>% 
  bind_rows(all_results_LOCATION_2023) %>% 
  bind_rows(all_results_LOCATION_2024) %>% 
  filter(!treatment %in% c("lsd_value", "p_value"))
```

```{r}
p2_1 <- all_results_LOCATION_year %>%
  filter(trt %in% c("yield_bu_acre")) %>%
  mutate(trt = recode(trt,
  "yield_bu_acre" = "Yield (Bu/are)")) %>%
  ggplot(aes(treatment, mean_r)) +
  geom_bar(aes(fill = treatment), stat = "identity", width = 0.5) +
  facet_grid(year ~ location, scales = "free_y") +
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2)+
  geom_text(aes(label = paste0(gr)), vjust = -1, size =5, show.legend = FALSE)+
  scale_y_continuous(expand = expansion(mult = c(0, 0.40)))+
  scale_fill_manual(values = c("#999999", "#336600", "#996600"),
...
```{r}
lsd_test_f2_06(cleaned_data_imputed %>% 
                 mutate(treatment = cultivar, 
                        group1 = as.factor(year),
...
```{r}
source("myFunctions2.R")
# Step 1: Prepare your data with correct columns
cleaned_input <- cleaned_data_imputed %>%
  mutate(
    treatment = cultivar,
    group = year,
    outcome = yield_bu_acre
...
# Step 2: Apply lsd_test_09_lsd_table to each location
results_by_location <- cleaned_input %>%
  group_by(location) %>%
  group_split() %>%
  map_df(~ {
    df <- .
    loc <- unique(df$location)
    result <- lsd_test_09_lsd_table(df)
    result$location <- loc  # add location column
    result
  })

# Step 3: draw plot
results_by_location %>% 
  mutate(treatment = fct_relevel(treatment, "CDC_Austenson", "AB_Hague", "Esma")) %>% 
ggplot(aes(as.factor(group), mean_r, fill = treatment)) +
  geom_bar(stat = "identity", width = 0.5, position = position_dodge(width = 0.7)) +
  facet_grid(. ~ location, scales = "free_x") +
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2,
                position = position_dodge(width = 0.7))+
  geom_text(aes(label = paste0(gr)), vjust = -1, size =4, show.legend = FALSE,
            position = position_dodge(width = 0.7))+
 ...
 theme(strip.text.y = element_text(size = 12, face = "bold", angle = 0), 
  ...
  coord_cartesian(clip = "off")+
  labs(x = "", y = "Yield (Bu/acre)", fill = "")
ggsave("Final_Report/figure/Year_Variety_impact_yieldBuAcre.png", width = 8, height = 4, dpi = 200)
```
...
ggplot(aes(as.factor(group), mean_r, fill = treatment)) +
 ...
 theme(strip.text.y = element_text(size = 12, face = "bold", angle = 0), 
  ...
  coord_cartesian(clip = "off")+
  labs(x = "", y = "Yield (kg/ha)", fill = "")
ggsave("Final_Report/figure/Year_Variety_impact_yieldkgha.png", width = 8, height = 4, dpi = 200)
```
...
ggplot(aes(as.factor(group), mean_r, fill = treatment)) +
 ...
 theme(strip.text.y = element_text(size = 12, face = "bold", angle = 0), 
  ...
  coord_cartesian(clip = "off")+
  labs(x = "", y = "Plant Height (cm)", fill = "")
ggsave("Final_Report/figure/Year_Variety_impact_plantHight.png", width = 8, height = 4, dpi = 200)
```

```{r}
source("myFunctions2.R")
# Step 1: Prepare your data with correct columns
cleaned_input_lod1 <- cleaned_data_imputed %>%
  mutate(
    treatment = cultivar,
    group = year,
    outcome = lodging_index_1
  )

cleaned_input_lod2 <- cleaned_data_imputed %>%
  mutate(
    treatment = cultivar,
    group = year,
    outcome = lodging_index_2
  )

cleaned_input_lod3 <- cleaned_data_imputed %>%
  mutate(
    treatment = cultivar,
    group = year,
    outcome = lodging_index_3
  )

cleaned_input_lodavg <- cleaned_data_imputed %>%
  mutate(
    treatment = cultivar,
    group = year,
    outcome = lodging_avg
...
# Step 2: Apply lsd_test_09_lsd_table to each location
results_by_location_lod1 <- cleaned_input_lod1 %>%
  group_by(location) %>%
  group_split() %>%
  map_df(~ {
    df <- .
  ...
  mutate(lodging = "lodging_index_1")

results_by_location_lod2 <- cleaned_input_lod2 %>%
  group_by(location) %>%
  group_split() %>%
  map_df(~ {
    df <- .
  ...
  mutate(lodging = "lodging_index_2")

results_by_location_lod3 <- cleaned_input_lod3 %>%
  group_by(location) %>%
  group_split() %>%
  map_df(~ {
    df <- .
  ...
  mutate(lodging = "lodging_index_3")

results_by_location_lodavg <- cleaned_input_lodavg %>%
  group_by(location) %>%
  group_split() %>%
  map_df(~ {
    df <- .
  ...
  mutate(lodging = "lodging_avg")

results_by_location <- results_by_location_lod1 %>% 
  bind_rows(results_by_location_lod2) %>% 
  bind_rows(results_by_location_lod3) %>% 
  bind_rows(results_by_location_lodavg) %>% 
  mutate(lodging = recode(lodging,
                      "lodging_index_1" = "Soft Dough",
                      "lodging_index_2" = "Hard Dough",
                      "lodging_index_3" = "Physiological Maturity",
                      "lodging_avg" = "Average"
...
ggplot(aes(as.factor(group), mean_r, fill = treatment)) +
  geom_bar(stat = "identity", width = 0.5, position = position_dodge(width = 0.7)) +
  facet_grid(location ~ lodging, scales = "free_x") +
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2,
                position = position_dodge(width = 0.7))+
  geom_text(aes(label = paste0(gr)), vjust = -1, size =3, show.legend = FALSE,
            position = position_dodge(width = 0.7))+
  scale_y_continuous(expand = expansion(mult = c(0, 0.20)))+
  scale_fill_manual(values = c("#999999", "#336600", "#996600"),
                    labels = c("AB Hague", "CDC Austenson", "Esma")
 ...
 theme(strip.text.y = element_text(size = 9, face = "bold", angle = 0), 
  ...
  coord_cartesian(clip = "off")+
  labs(x = "", y = "Lodging Index (%)", fill = "")
ggsave("Final_Report/figure/Year_Variety_impact_lodging.png", width = 8, height = 4, dpi = 200)
```

```{r}
p3_1 <- all_results_LOCATION_year %>%
  filter(trt %in% c("avg_height_at_phys_maturity")) %>%
  mutate(trt = recode(trt,
  "avg_height_at_phys_maturity" = "Plant height (cm)"
)) %>%
...
#| warning: false
#| # Vector of locations
cleaned_data_imputed_2022 <- cleaned_data_imputed %>% filter(year == 2022)
cleaned_data_imputed_2023 <- cleaned_data_imputed %>% filter(year == 2023)
cleaned_data_imputed_2024 <- cleaned_data_imputed %>% filter(year == 2024)
...
# Loop over all combinations of locations and treatments
all_results_02_2022 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_2022, ..1, ..2)) %>% 
  mutate(year = 2022)
all_results_02_2023 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_2023, ..1, ..2)) %>% 
  mutate(year = 2023)
all_results_02_2024 <- crossing(loc = locations_2, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_2024, ..1, ..2)) %>% 
  mutate(year = 2024)
all_results_02_year <- all_results_02_2022 %>% 
  bind_rows(all_results_02_2023) %>% 
  bind_rows(all_results_02_2024)
...
p4_yield <- data_graph %>%
  filter(trt %in% c("yield_bu_acre")) %>%
 # mutate(trt = fct_relevel(trt, "Hard Dough", "Physiological Maturity", "Average", after = 1)) %>% 
  ggplot(aes(variety, mean_r)) +
  geom_bar(aes(fill = variety), stat = "identity", show.legend = TRUE, width = 0.5) +
  facet_grid(. ~ location) + #, scales = "free_y"
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2)+
  ...
  labs(x = "Variety", y = "Yield (Bu/acre)", fill = "", title = "Variety impact on yield")
p4_yield
ggsave("Final_Report/figure/Variety_impact_yield.png", width = 8, height = 4, dpi = 200)
```
...
# Loop over all combinations of locations and treatments
all_results_01_2022 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_2022, ..1, ..2)) %>% 
  rename(variety = treatment)%>% 
  mutate(year = 2022)

all_results_01_2023 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_2023, ..1, ..2)) %>% 
  rename(variety = treatment)%>% 
  mutate(year = 2023)

all_results_01_2024 <- crossing(loc = locations_2, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_2024, ..1, ..2)) %>% 
  rename(variety = treatment) %>% 
  mutate(year = 2024)

all_results_01_year <- all_results_01_2022 %>% 
  bind_rows(all_results_01_2023) %>% 
  bind_rows(all_results_01_2024)
#writexl::write_xlsx(all_results_01_2022, "Final_Report/Data/all_results_location_variety_2022.xlsx")
```

### Graphs

```{r}
data_graph_01_year <- all_results_01_year %>% 
  select(-c(mean_se, result)) %>% 
  filter(!(variety %in% c("lsd_value", "p_value")))

data_graph_lsd_pvalue_year <- all_results_01_year %>% 
  select(-c(mean_se, result, SE_r, gr)) %>% 
  filter(variety %in% c("lsd_value", "p_value")) %>% 
  rename(test = variety,
         test_value = mean_r) %>% 
...
data_graph_year <- data_graph_01_year %>% 
  left_join(data_graph_lsd_pvalue_year)
```

```{r}
p4_2 <- data_graph_year %>%
 ...
 # scale_y_continuous(breaks = seq(0,14,2))+
  labs(x = "Variety", y = "Lodging Index (%)", fill = "", title = "a) Lodging at different growth stages at Falher (2023)",
       caption = "(No lodging occured in 2022 and no data available for 2024)")
p4_2
ggsave("Final_Report/figure/Fahler_Variety_impact_lodging_yearWise.png", width = 8, height = 4, dpi = 200)
```

```{r}
p4_3 <- data_graph_year %>%
  ...
  ggplot(aes(variety, mean_r)) +
  geom_bar(aes(fill = variety), stat = "identity", show.legend = TRUE, width = 0.5) +
  facet_grid(year ~ trt) + #, scales = "free_y"
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2)+
  geom_text(aes(label = gr, y = mean_r + SE_r + 0.1), vjust = -0.3, size =5, show.legend = FALSE)+
  scale_fill_manual(values = c("#999999", "#336600", "#996600"),
                    labels = c("AB Hague", "CDC Austenson", "Esma"))+
 ...
 # coord_cartesian(ylim = c(0,9))+
 # scale_y_continuous(breaks = seq(0,14,2))+
  labs(x = "Variety", y = "Lodging Index (%)", fill = "", title = "b) Lodging at different growth stages at Lethbridge")
p4_3
ggsave("Final_Report/figure/Lethbridger_Variety_impact_lodging_yearWise.png", width = 7, height = 4, dpi = 200)
```
...
p4_3 <- data_graph_year %>%
 ...
 # coord_cartesian(ylim = c(0,9))+
 # scale_y_continuous(breaks = seq(0,14,2))+
  labs(x = "Variety", y = "Lodging Index (%)", fill = "", title = "c) Lodging at different growth stages at Vermilion (2022)",
       caption = "(No lodging occured in 2023 and 2024)")
p4_3
ggsave("Final_Report/figure/Vermilion_Variety_impact_lodging_yearWise.png", width = 7, height = 4, dpi = 300)
```
...
mutate(treatment = case_when(
  ...
  facet_grid(location ~ trt) +#, scales = "free_y") +
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2)+
  geom_text(aes(label = gr, y = mean_r + SE_r + 0.3), vjust = -0.3, size =3, show.legend = FALSE)+
 ...
 # coord_cartesian(ylim = c(0,12))+
  ...
  labs(x = "Treatment", y = "Lodging Index (%)", fill = "", title = "d) Lodging at different growth stages (Average of three years)")
p5
...
```{r}
data_graph_02_year <- all_results_02_year %>% 
  select(-c(mean_se, result)) %>% 
  filter(!(treatment %in% c("lsd_value", "p_value")))

data_graph_lsd_pvalue_02_year <- all_results_02_year %>% 
  select(-c(mean_se, result, SE_r, gr)) %>% 
  filter(treatment %in% c("lsd_value", "p_value")) %>% 
  rename(test = treatment,
         test_value = mean_r) %>% 
...
data_graph_02_year <- data_graph_02_year %>% 
  left_join(data_graph_lsd_pvalue_02_year)
```

```{r}
p5_1 <- data_graph_02_year %>%
  filter(location == "Falher") %>% 
  filter(year == 2023) %>% 
   filter(trt %in% c("lodging_index_1", "lodging_index_2", "lodging_index_3", "lodging_avg")) %>%
  mutate(trt = recode(trt,
...
mutate(treatment = case_when(
 ...
 # coord_cartesian(ylim = c(0,12))+
  ...
  labs(x = "Treatment", y = "Lodging Index (%)", fill = "", title = "a) Lodging at different growth stages at Falher (2023)",
       caption = "(No lodging occured in 2022 and no data available for 2024)")
p5_1
ggsave("Final_Report/figure/Fahler_PGR_impact_lodging_yearWise.png", width = 8, height = 4, dpi = 200)
```

```{r}
p5_2 <- data_graph_02_year %>%
  filter(location == "Lethbridge") %>% 
   filter(trt %in% c("lodging_index_1", "lodging_index_2", "lodging_index_3", "lodging_avg")) %>%
  mutate(trt = recode(trt,
                      "lodging_index_1" = "Soft Dough",
...
mutate(treatment = case_when(
 ...
 # coord_cartesian(ylim = c(0,12))+
  #scale_y_continuous(breaks = seq(0,9,1))+
  labs(x = "Treatment", y = "Lodging Index (%)", fill = "", title = "b) Lodging at different growth stages at Lethbridge")
p5_2
ggsave("Final_Report/figure/Lethbridge_PGR_impact_lodging_yearWise.png", width = 8, height = 4, dpi = 200)
```

```{r}
p5_3 <- data_graph_02_year %>%
  filter(location == "Vermilion") %>% 
  filter(year == 2022) %>% 
   filter(trt %in% c("lodging_index_1", "lodging_index_2", "lodging_index_3", "lodging_avg")) %>%
  mutate(trt = recode(trt,
...
mutate(treatment = case_when(
  ...
  #scale_y_continuous(breaks = seq(0,9,1))+
  labs(x = "Treatment", y = "Lodging Index (%)", fill = "", title = "c) Lodging at different growth stages at Vermilion (2022)",
       caption = "(No lodging occured in 2023 and 2024)")
p5_3
ggsave("Final_Report/figure/Vermilion_PGR_impact_lodging_yearWise.png", width = 8, height = 4, dpi = 200)
```
...
mutate(treatment = case_when(
  ...
  labs(x = "Treatment", y = "Yield (Bu/acre)", fill = "", title = "AB Hague")
p9_1
ggsave("Final_Report/figure/ABHague_PRG_impact_yieldBuAcre_year.png", width = 8, height = 4, dpi = 200)
```
...
cleaned_data_imputed_Esma_2022 <- cleaned_data_imputed %>% filter(year == 2022) %>% 
  filter(cultivar == "Esma")

cleaned_data_imputed_Esma_2023 <- cleaned_data_imputed %>% filter(year == 2023) %>% 
  filter(cultivar == "Esma")

cleaned_data_imputed_Esma_2024 <- cleaned_data_imputed %>% filter(year == 2024) %>% 
  filter(cultivar == "Esma")


cleaned_data_imputed_ABHague_2022 <- cleaned_data_imputed %>% filter(year == 2022) %>% 
  filter(cultivar == "AB_Hague")

cleaned_data_imputed_ABHague_2023 <- cleaned_data_imputed %>% filter(year == 2023) %>% 
  filter(cultivar == "AB_Hague")

cleaned_data_imputed_ABHague_2024 <- cleaned_data_imputed %>% filter(year == 2024) %>% 
  filter(cultivar == "AB_Hague")

cleaned_data_imputed_CDC_Austenson_2022 <- cleaned_data_imputed %>% filter(year == 2022) %>% 
  filter(cultivar == "CDC_Austenson")

cleaned_data_imputed_CDC_Austenson_2023 <- cleaned_data_imputed %>% filter(year == 2023) %>% 
  filter(cultivar == "CDC_Austenson")

cleaned_data_imputed_CDC_Austenson_2024 <- cleaned_data_imputed %>% filter(year == 2024) %>% 
  filter(cultivar == "CDC_Austenson")
...
# Loop over all combinations of locations and treatments
all_results_Esma_2022 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_Esma_2022, ..1, ..2)) %>% 
  mutate(year = 2022)

all_results_Esma_2023 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_Esma_2023, ..1, ..2))%>% 
  mutate(year = 2023)

all_results_Esma_2024 <- crossing(loc = locations_2, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_Esma_2024, ..1, ..2))%>% 
  mutate(year = 2024)

all_results_Esma_year <- all_results_Esma_2022 %>% 
  bind_rows(all_results_Esma_2023) %>% 
  bind_rows(all_results_Esma_2024)



all_results_ABHague_2022 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_ABHague_2022, ..1, ..2)) %>% 
  mutate(year = 2022)

all_results_ABHague_2023 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_ABHague_2023, ..1, ..2))%>% 
  mutate(year = 2023)

all_results_ABHague_2024 <- crossing(loc = locations_2, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_ABHague_2024, ..1, ..2))%>% 
  mutate(year = 2024)

all_results_ABHague_year <- all_results_ABHague_2022 %>% 
  bind_rows(all_results_ABHague_2023) %>% 
  bind_rows(all_results_ABHague_2024)



all_results_CDC_Austenson_2022 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_CDC_Austenson_2022, ..1, ..2)) %>% 
  mutate(year = 2022)

all_results_CDC_Austenson_2023 <- crossing(loc = locations, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_CDC_Austenson_2023, ..1, ..2))%>% 
  mutate(year = 2023)

all_results_CDC_Austenson_2024 <- crossing(loc = locations_2, trt = treatments) %>%
  pmap_dfr(~run_lsd_test(cleaned_data_imputed_CDC_Austenson_2024, ..1, ..2))%>% 
  mutate(year = 2024)

all_results_CDC_Austenson_year <- all_results_CDC_Austenson_2022 %>% 
  bind_rows(all_results_CDC_Austenson_2023) %>% 
  bind_rows(all_results_CDC_Austenson_2024)
```

```{r}
p13_1 <- all_results_Esma_year %>%
   filter(trt %in% c("yield_bu_acre")) %>%
  mutate(trt = recode(trt,
  "yield_bu_acre" = "Yield (Bu/are)")) %>% 
mutate(treatment = case_when(
  ...
  ggplot(aes(treatment, mean_r, fill = as.factor(year))) +
  geom_bar(stat = "identity", show.legend = TRUE, width = 0.5,
           position = position_dodge(width = 0.7)) +
  facet_grid(. ~ location) +
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2,
                position = position_dodge(width = 0.7))+
  geom_text(aes(label = gr, y = mean_r + SE_r + 0.3), vjust = -0.3, size =4, show.legend = FALSE, position = position_dodge(width = 0.7))+
  scale_fill_manual(values = c("#999999", "#336600", "#996600", "#66FFFF", "#0066CC", "#660033", "#009999", "#000000", "#FF3300"))+
  scale_y_continuous(expand = expansion(mult = c(0, 0.30)))+
   theme_bw()+
  theme(strip.text.y = element_text(size = 12, face = "bold", angle = 0), 
  ...
  labs(x = "Treatment", y = "Yield (Bu/acre)", fill = "", title = "Esma")
p13_1
ggsave("Final_Report/figure/Esma_PRG_impact_yieldBuAcre_year_location.png", width = 8, height = 4, dpi = 200)
```

```{r}
p13_2 <- all_results_ABHague_year %>%
   filter(trt %in% c("yield_bu_acre")) %>%
  mutate(trt = recode(trt,
  "yield_bu_acre" = "Yield (Bu/are)")) %>% 
mutate(treatment = case_when(
  ...
  ggplot(aes(treatment, mean_r)) +
  geom_bar(aes(fill = treatment), stat = "identity", show.legend = TRUE, width = 0.5) +
  facet_grid(year ~ location, space = "free") +
  geom_errorbar(aes(ymin = mean_r - SE_r, ymax = mean_r + SE_r), width = 0.2)+
  geom_text(aes(label = gr, y = mean_r + SE_r + 0.3), vjust = -0.3, size =4, show.legend = FALSE)+
  scale_fill_manual(values = c("#999999", "#336600", "#996600", "#66FFFF", "#0066CC", "#660033", "#009999", "#000000", "#FF3300"))+
  scale_y_continuous(expand = expansion(mult = c(0, 0.30)))+
  ...
  labs(x = "Treatment", y = "Yield (Bu/acre)", fill = "", title = "AB Hague")
p13_2
ggsave("Final_Report/figure/ABHague_PRG_impact_yieldBuAcre_year_location.png", width = 8, height = 4, dpi = 200)
```

```{r}
p13_3 <- all_results_CDC_Austenson_year %>%
   filter(trt %in% c("yield_bu_acre")) %>%
  mutate(trt = recode(trt,
  "yield_bu_acre" = "Yield (Bu/are)")) %>% 
mutate(treatment = case_when(
...
p13_3
ggsave("Final_Report/figure/CDC_Austenson_PRG_impact_yieldBuAcre_year_location.png", width = 8, height = 4, dpi = 200)
```
...
ggplot(model_results, aes(x = estimate, y = term, shape = effect, color = significant_or_not)) +
  ...
  labs(title = "",
       x = "Model Estimated Effect (Bu/ac)", y = "", #caption = "Yield Mixed Model Effects with Confidence Intervals",
...
ggplot(model_results, aes(x = estimate, y = term, shape = effect, color = significant_or_not)) +
  ...
  scale_color_manual(values = c("gray60", "gray10", "green4")) +  # Different colors for fixed and random
  labs(title = "",
       x = "Model Estimated Effect (Kg/ha)", y = "", #caption = "Yield Mixed Model Effects with Confidence Intervals",
       shape = "Effect Type", color = "P.Value") +
...
# Fit the mixed model with year & location as random effects
reg_mod_lodging <- lmer(lodging_avg ~ cultivar + pgr_trt_name + 
                (1 | year) + (1 | location), 
                data = cleaned_data_imputed %>% mutate(pgr_trt_name = fct_relevel(pgr_trt_name, "No_PGR"),
                                       cultivar = fct_relevel(cultivar, "CDC_Austenson")))

# Extract fixed effects with p-values
fixed_effects <- broom.mixed::tidy(reg_mod_lodging, effects = "fixed") %>%
  filter(term != "(Intercept)")

# Extract confidence intervals for fixed effects
conf_int <- confint(reg_mod_lodging, method = "Wald") %>%  # Compute 95% CIs
  as.data.frame() %>%
  rownames_to_column("term") %>%
  filter(term %in% fixed_effects$term) %>%
  rename(ci_lower = `2.5 %`, ci_upper = `97.5 %`)
...
# Extract random effects (variance components) - No p-values here
random_effects <- broom.mixed::tidy(reg_mod_lodging, effects = "ran_pars") %>%
  select(-term) %>% 
  rename(term = group) %>% 
  mutate(
    term = str_replace(term, "sd__", ""),  # Clean term names
...
### Mixed modeling with interactions
...
model_results <- model_results %>%
  ...
  mutate(category = case_when(
    grepl("location", term) ~ "Location",
    grepl("year", term) ~ "Year",
    term %in% c("cultivarEsma", "cultivarAB_Hague") ~ "Cultivar",
  ...
  mutate(term = case_when(
    term == "CDC_Austenson" ~ "CDC Austenson",
...
ggplot(model_results, aes(x = estimate, y = term, shape = effect, color = significant_or_not)) +
  ...
  #theme_minimal() +
  theme(strip.text.y = element_text(angle = 0))
ggsave("Final_Report/figure/Yield_mixed_modeling_interaction.png", width = 8, height = 8, dpi = 600)
```
