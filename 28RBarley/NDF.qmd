...
reg_mod_NDF <- lmer(ndf_percent_dm ~ cultivar + pgr_trt_name + 
                (1 | year) + (1 | location), 
...
fixed_effects <- broom.mixed::tidy(reg_mod_NDF, effects = "fixed") %>%
  filter(term != "(Intercept)")

# Extract confidence intervals for fixed effects
conf_int <- confint(reg_mod_NDF, method = "Wald") %>%  # Compute 95% CIs
  as.data.frame() %>%
...
random_effects <- broom.mixed::tidy(reg_mod_NDF, effects = "ran_pars") %>%
  select(-term) %>% 
  rename(term = group) %>% 
  mutate(
...
ggplot(model_results_height, aes(x = estimate, y = term, shape = effect, color = significant_or_not)) +
  ...
  labs(title = "", caption = "NDF Mixed Model Effects with Confidence Intervals",
...
ggsave("figure/NDF/NDF_modeling.png", width = 8, height = 5, dpi = 600)
```