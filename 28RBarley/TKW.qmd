...
reg_mod_TKW <- lmer(tkw_avg_adjusted_to_13_5_percent_moisture ~ cultivar + pgr_trt_name + 
                (1 | year) + (1 | location), 
...
fixed_effects <- broom.mixed::tidy(reg_mod_TKW, effects = "fixed") %>%
  filter(term != "(Intercept)")

# Extract confidence intervals for fixed effects
conf_int <- confint(reg_mod_TKW, method = "Wald") %>%  # Compute 95% CIs
  as.data.frame() %>%
...
random_effects <- broom.mixed::tidy(reg_mod_TKW, effects = "ran_pars") %>%
  select(-term) %>% 
  rename(term = group) %>% 
  mutate(
...
ggplot(model_results_height, aes(x = estimate, y = term, shape = effect, color = significant_or_not)) +
  ...
  labs(title = "", caption = "TKW Mixed Model Effects with Confidence Intervals",
...
ggsave("figure/TKW/TKW_modeling.png", width = 8, height = 5, dpi = 600)
```