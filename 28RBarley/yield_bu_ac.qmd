...
reg_mod <- lmer(yield_adj_to_13_5_percent_bu ~ cultivar + pgr_trt_name + 
                (1 | year) + (1 | location), 
...
ggsave("figure/yield_bu_ac/Yield_mixed_modeling.png", width = 8, height = 5, dpi = 600)
```
...
p1_1 <- lsd_test_02_mod(data_stat %>% 
              ...
              mutate(treatment = factor(year),
                     outcome = yield_adj_to_13_5_percent_bu,
                     group = cultivar))+
...
```{r}
p2_1 <- lsd_test_02_mod(data_stat %>% 
              filter(location == "Lethbridge") %>% 
              mutate(treatment = factor(year),
                     outcome = yield_adj_to_13_5_percent,
                     group = cultivar))+
...
```{r}
p3_1 <- lsd_test_02_mod(data_stat %>% 
              filter(location == "Falher") %>% 
              mutate(treatment = factor(year),
                     outcome = yield_adj_to_13_5_percent,
                     group = cultivar))+
...
p1 <- lsd_test_01_mod(data_stat %>% 
  ...
  coord_cartesian(ylim = c(0,8000))+
   theme(plot.title = element_text(hjust = 0.5),
    axis.text.x = element_text(vjust = 0.5,
    angle = 15)) +labs(x = NULL, y = NULL)+
  labs(x= "", y = "Yield (Bu/ac)", title = "Vermilion")
p1
```

```{r}
p1 + p3 + p2
ggsave("figure/yield_bu_ac/Yield_varietyy_locations.png", width = 8, height = 5, dpi = 600)
```
